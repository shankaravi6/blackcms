...
const AddEditPremPromptData = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        const url = id
          ? `${BASE_URL}/api/data/${"prem_promptrix_data"}/${id}`
          : `${BASE_URL}/api/data/${"prem_promptrix_data"}`;
        ...
        navigate("/prempromptrix"); // Navigate to the list page after successful submission
      } catch (error) {
  ...
  useEffect(() => {
    if (id) {
      axios
        .get(`${BASE_URL}/api/data/${"prem_promptrix_data"}/${id}`)
        .then((response) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      setIsSubmitting(true);
      await axios.delete(`${BASE_URL}/api/data/${"prem_promptrix_data"}/${id}`);
      setIsSubmitting(false);
      setOpenDeleteDialog(false); // Close the dialog after deletion
      navigate("/prempromptrix"); // Navigate back to the list page
    } catch (error) {
    ...
    <Grid container spacing={3} sx={{ padding: 3 }}>
      <Grid item xs={12}>
        <Button
          ...
          onClick={() => navigate("/prempromptrix")}
        >
          Back
        </Button>
      </Grid>
      ...
      >
        <Typography variant="h5" gutterBottom align="center" sx={{ mb: 3 }}>
          {id ? "Update Premium Prompt" : "Add New Premium Prompt"}
        </Typography>
...
export default AddEditPremPromptData;
